...
@router.post("/")
async def subscribe(subscription: Subscription, request: Request, background_tasks: BackgroundTasks, response_model = Response):
...
@router.get('/desinscrever/confirmacao/{id}', response_model = Response)
...
async def unsubscribe(request_data: UnsubscribeRequest, request: Request, background_tasks: BackgroundTasks, response_model = Response):
